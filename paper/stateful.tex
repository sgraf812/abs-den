...
Recall \Cref{fig:syntax}; it defines the syntax of an untyped lambda
...
The key to avoiding algebraic domains for our work is use of a total type theory
with \emph{guarded recursive types}, such as Guarded Dependent Type Theory
(GDTT) \citep{gdtt}.%
...
Thus, GDTT walks a fine line:
The theory guarantees that all well-typed functions (naturally) correspond
to continuous functions, while it also allows for embedding of a sufficiently
expressive restriction of endofunctions for denotational semantics.
...
\[\begin{array}{c}
 \begin{array}{rrclclrrclcl}
  ...
  &
  \text{Heaps}         & μ   & ∈ & \Heaps         & =      & \Addresses \pfun \StateD
  \\
  \\[-0.5em]
 \end{array} \\
 \begin{array}{rrclclrrclcl}
 ...
 \end{array} \\
 \begin{array}{rrclcl}
  \text{Stateful value} & v & ∈ & \StateV & ::= & \FunV(f ∈ (\StateD \to \later\StateD)) \mid \ConV(K,\many{d^{\later}}^{α_K}) \\
 \end{array} \\
  \\[-0.5em]
\end{array}\]
\[\begin{array}{c}
 \begin{array}{rcl}
  \multicolumn{3}{c}{ \ruleform{
    \begin{array}{c}
      (\betastep) : \STraces \to (\States \pfun \later\STraces) \to \STraces \quad  \ret : (\Val \times \StateV) \to \StateD \\
      \deref : \Addresses \to \StateD \quad \apply : \STraces \to \StateD \to \STraces \\
      \select : \STraces \to ((\StateD)^{α_K} \pfun \STraces)^* \to \STraces \\
    \end{array}
  }} \\
  \\[-0.5em]
  ...
  \deref(\pa)(μ) & = & μ(\pa)(μ) \betastep \fn{((\lbl,v),μ)}{\idiom{\goodend{((\lbl,v),μ[\pa ↦ \ret(\lbl,v)])}}} \\
  ...
  \semst{\px}_ρ(μ) & = & \begin{cases}
    (\px,μ) \cons \idiom{ρ(\px)(μ)} & \px ∈ \dom(ρ) \\
  ...
  \semst{\Let{\px}{\pe_1}{\pe_2}}_ρ(μ) & = & \begin{letarray}
    \text{let} & ρ' = ρ[\px ↦ \deref(\pa)] \quad \text{where $\pa \not∈ \dom(μ)$} \\
               & μ^\later = \idiom{μ[\pa ↦ \semst{\pe_1}_{ρ'}]} \\
    \text{in}  & (\Let{\px}{\pe_1}{\pe_2},μ) \cons \idiom{\semst{\pe_2}_{ρ'}(μ^\later)} \\
  \end{letarray} \\
  \\[-0.5em]
  \semst{\slbl K~\many{\px}}_ρ & = & \ret(\lbl,\ConV(K,\many{\idiom{\semst{\px}_ρ}})) \\
  \\[-0.5em]
  \semst{\pe@(\Case{\pe_s}{\Sel[r]})}_ρ(μ) & = & (\pe,μ) \cons \idiom{\select(\semst{\pe_s}_ρ(μ),\many{\fn{\many{d}^{α_K}}{\semst{\pe_r}_{ρ[\many{\px↦d}]}}})}  \\
 \end{array}
  \\[-0.5em]
\end{array}\]
