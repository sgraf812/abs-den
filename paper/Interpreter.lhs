...
We take a program's meaning to be a \emph{trace} of its execution, recording the steps
that a denotational semantics usually abstracts away.
This is still a denotational semantics, compositional and parameterised over its domain,
giving a single generic \emph{denotational interpreter}.
...
definition in Lean (\Cref{sec:mech-domain}).
...
The step events fall into two kinds, following \citet{Harper:16}.
The \emph{search transitions} |Let1|, |App1|, |Case1|, and each |Look x| fire as
evaluation enters a construct or forces a variable, potentially pushing a continuation
...
\smallskip
\noindent
The example we have seen suggests that |evalNeed2| indeed defines
