...
\begin{figure}
\[\begin{array}{c}
 \ruleform{ \fin(τ) \qquad A ⊦ d \qquad A ⊦ τ_1 \sim τ_2 \qquad A ⊦ μ_1 \approx μ_2 \qquad d_1 \equiv d_2}
 \\
 \\[-0.5em]
 ...
 \inferrule*[right=\eqdenot]
    % Why not A ⊦ d_1,d_2 outside the forall?
    % Because then we must pick the smallest such A!
    % Otherwise this predicate is too weak. Nasty.
    {\forall A,μ_1,μ_2.\ A ⊦ d_1,d_2 \wedge A ⊦ μ_1 \approx μ_2  ⟹  A ⊦ d_1(μ_1) \sim d_2(μ_2)}
    {d_1 \equiv d_2}
\end{array}\]
...
ruling out the local address $\pa_1 \not∈ A$.
...
The $A \vdash μ_1 \approx μ_2$ judgment (which is reminiscent of the frame rule
in separation logic) ensures that the heaps are extended compatibly.
...
\begin{itemize}
  ...
  \item \textbf{Case $μ_1'(\pa) \not= μ_2'(\pa)$}:
    ...
    % This is nice, because we didn't need the heap forcing lemma to show
    % this.
\end{itemize}
\end{proof}
