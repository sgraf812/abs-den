...
\begin{definition}[Address domain, trusting]
  ...
  \begin{itemize}
    \item If $v = \FunV(f)$ for some $f$, then for all trusting $d'$ such that $A ⊦ d'$, $A ⊦ f(d')(μ)$.
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item \textbf{Case $\Lam{px}{\pe'}$}:
      It is $\bigstep{\semevt{\pe}_ρ}{μ}{\FunV(f)}{μ}$ for any $A ⊆ \dom(μ)$ where
      $f \triangleq \fn{d}{\AppEA(\px↦d) \cons \semevt{\pe}_{ρ[\px↦d]}}$, so we
      assume that there is a trusting $d$ such that $A ⊦ d$.
      (Clearly, such a $d$ exists, for example, $A ⊦ \fn{\wild}{\stuckend{}}$ for any $A$.)
      We have to show that $A ⊦ f(d)(μ)$ which follows by the induction hypothesis,
      because $\semevt{\pe'}_ρ[\px↦d]$ is trusting when $d$ is.
      At the same time, this means that $A = \adom(\semevt{\pe'}_ρ[\px↦d])$ is the
      smallest such $A$.
