...
$\semevt{\wild}$ but they apply just the same for the vanilla $\semvan{\wild}$.
...
\begin{definition}[Lazy domain]
  ...
  \begin{itemize}
    \item When $v = \FunV(f)$ (for some $f$), then $f(\pa)$ is lazy.
  \end{itemize}
\end{definition}
...
pre-allocate any denotation into the heap they want to due to laziness.
...
Let us briefly summarise a few results \wrt to compositionality.
As compositionality is a property referring to contexts, we
...
We think it would be simple to prove that $\semevt{\wild}$ is
\emph{compositional} \wrt definitional equality $=$ on $\EventD$:
