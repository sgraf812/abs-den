...
\begin{toappendix}
\begin{lemma}
  \label{thm:force-bisimilar}
  Let $μ_1,μ_2$ be lazy heaps with $μ_1 \forcesto μ_2$. Then
  \begin{enumerate}
    \item For all lazy $d$, we have $\dom(μ_1) ⊦ d(μ_1) \sim d(μ_2)$.
...
%  This lemma makes essential use of the Postpone and Speculate properties.
  By mixed induction/Löb induction.
  That is, we assume both that the predicate holds \emph{later} as well as
  \emph{now} for any sub-occurrence. %WTF
  \begin{enumerate}
    \item
...
%      at $μ_1$.
      ...
      judgments hold by the Speculate and Postpone properties and we have
      $μ_1' \forcesto μ_2'$.
      Hence it suffices to show
      ${\dom(μ_1) ⊦ \goodend{v,μ_1'} \sim \goodend{v,μ_2'}}$.

      We have $\later (\many{\dom(μ_1') ⊦ μ_1'(\pa_1)(μ_1') \sim μ_2'(\pa_2)(μ_2')})$
      by the induction hypothesis applied to $μ_1' \forcesto μ_2'$.

      For the $\ConV$ case, we can relax the address space from
      $\dom(μ_1')$ to $\dom(μ_1)$ via \Cref{thm:weaken-address-space} to show
      the goal.

      For the $\FunV(f)$ case, we fix $\pa ∈ \dom(μ_1)$ and have to show that
      $\dom(μ_1) ⊦ f(\pa)(μ_1') \sim f(\pa)(μ_2')$.
      % Can't do Löb induction without later!!!
      % And we really do need regular induction here, I think, because otherwise
      % the predicate is not strong enough.
      % Hmm. Perhaps we *can* afford a later, because the App case makes an
      % app_1 step before recursion.
      % It's also not possible to show it inductively, because we have nothing
      % to work with regarding f(\pa)
    ...
    \item
      If $μ_1(\pa) = μ_2(\pa)$ then (2) follows from (1),
      so let $μ_1(\pa) \not= μ_2(\pa)$.
      From $μ_1 \forcesto μ_2$ it follows that $\bigstep{μ_1(\pa)}{μ_1}{v}{μ_1'}$
      for some $v,μ_1'$, $μ_2(\pa) = \memo(\pa,\ret(v))$ and $μ_1' \forcesto μ_2$.
      This implies that $\bigstep{μ_2(\pa)}{μ_2}{v}{μ_2}$.
  \end{enumerate}

  hence it suffices to show
  $\dom(μ_1) ⊦ μ_1(\pa)(μ_1) \sim μ_2(\pa)(μ_2)$.
  If either $μ_1(\pa)(μ_1)$ or $μ_2(\pa)(μ_2)$ is ultimately stuck,
  then so is the other, because it neither diverges nor evaluates due to
  laziness.
  In that case, we see that $\dom(μ_1) ⊦ μ_1(\pa)(μ_1) \sim μ_2(\pa)(μ_2)$.

  If either $μ_1(\pa)(μ_1)$ or $μ_2(\pa)(μ_2)$ diverges, then so does the other,
  in which case $\dom(μ_1) ⊦ μ_1(\pa)(μ_1) \sim μ_2(\pa)(μ_2)$.

  If either $\bigstep{μ_1(\pa)}{μ_1}{v}{μ_1'}$ or $\bigstep{μ_2(\pa)}{μ_2}{v}{μ_2'}$
  then both judgments hold by the Speculate and Postpone properties and $μ_1'
  \forcesto μ_2'$, so it suffices to show ${\dom(μ_1) ⊦ \goodend{v,μ_1'} \sim
  \goodend{v,μ_2'}}$.

  For the $\ConV$ case, we can show $\later (\many{\dom(μ_1') ⊦ μ_1'(\pa_1)(μ_1') \sim μ_2'(\pa_2)(μ_2')})$
  via Löb induction and relax the address space from $\dom(μ_1')$ to $\dom(μ_1)$
  via \Cref{thm:weaken-address-space} to show the goal.

  For the $\FunV(f)$ case, we fix an arbitrary $\pa ∈ \dom(μ_1)$ and have to
  show that $\dom(μ_1) ⊦ f(\pa)(μ_1') \sim f(\pa)(μ_2')$.
  Again, that follows via Löb induction, because $μ_1' \forcesto μ_2'$ and we
  may relax the address space from $\dom(μ_1')$ to $\dom(μ_1)$ via
  \Cref{thm:weaken-address-space}.
\end{proof}
...
\begin{proof}
  ...
  space from $\dom(μ_1')$ to $\dom(μ_1)$ via \Cref{thm:weaken-address-space}.
