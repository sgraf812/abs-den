...
Such an invariant is ``semantic'' in that it does not have an obvious syntactic
interpretation at all!%
\footnote{For the curious: We found proving \Cref{thm:dead-let} surprisingly
difficult for a syntactic deadness definition.}
...
\[
  \forall \pE.\ P(ρ(\pE),μ(\pE),A(\pE)) \Longrightarrow \len(\semevt{\pE}(φ_P(S_1))_{[]}([])) = ω \Longleftrightarrow \len(\semevt{\pE}(φ_P(S_2))_{[]}([])) = ω,
\]
...
around questions of \emph{full abstraction}~\citep{Plotkin:77}; clearly,
$(\equiv)$ is fully abstract.
