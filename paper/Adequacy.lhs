...
In this Section, we prove that the |evalName| and |evalNeed| instantiations of
our generic interpreter are indeed \emph{denotational semantics}.
...
Proving adequacy amounts to proving that the traces produced by |evalNeed|
correspond one-to-one to the set of what we call the \emph{maximal} (\ie,
longest possible) small-step traces of the corresponding lazy Krivine machine.
In particular, j
preserve termination properties and the precise list of transition |Event|s of
the Lazy Krivine traces in \Cref{fig:lk-semantics} they abstract.
...
one-to-one We show adequacy in this manner at |D (ByNeed T)|.
...
Balanced traces are a proper subset of the maximal small-step LK traces that ---
by nature of big-step semantics --- excludes stuck and diverging traces.
...
\begin{theoremrep}[Adequacy]
  ...
  Let |e| be a closed expression and let |τ := evalNeed e emp emp :: T _|\sven{Please don't use the unwrapper |unByNeed| as it is ugly and doesn't add much information. Just use $\mathcal{S}\llbracket e \rrbracket\epsilon$}.
  ...
  intuitive way to the transition rule name in the LK trace.
\end{theoremrep}
\begin{proof}
  A consequence of \Cref{thm:abs-length} and \Cref{thm:sem-correct}.
