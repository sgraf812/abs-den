...
\section{|evalNeed2| is a Semantics: Totality and Bisimulation}

In this section, we prove that |evalNeed2| is indeed a \emph{denotational semantics}.%
\footnote{Similar results for |evalName| should be derivative.}
Specifically, denotational semantics must be total and adequate.
An interpreter is \emph{total} if it is well-defined for every input
expression, and \emph{adequate} if it can be used to observe the termination
behavior of a reference semantics.
We prove totality by embedding |evalNeed2| in a guarded type theory,
and we prove adequacy by showing that the traces generated by |evalNeed2|
\emph{bisimulate} traces of the Lazy Krivine machine.
To our knowledge, |evalNeed2| is the first denotational call-by-need
semantics with a proof of bisimulation!
This is an important result because it allows us to switch between abstract
machine and denotational interpreter as needed, thus guaranteeing compatibility
