...
came.
This unguarded occurrence is not prohibited by the type construction rules of
guarded domain theory, although \citet[Section 9]{Birkedal:12} argue that it
might be enough to guard negative recursive occurrences for the type to exist.
Another possibility would be for the $\FunV$ value to carry the lambda-bound
variable as well so that the $\AppEE$ event can be built in $\apply$.
We will stick to the present formulation because it is conceptually cleaner.
...
extended correctness predicate for $\semevt{\wild}$ in \Cref{fig:semevt-correctness}.
...
our earlier examples from \Cref{sec:vanilla}, starting with $\Let{i}{\Lam{x}{x}}{i~i}$:
\[\begin{array}{ll}
