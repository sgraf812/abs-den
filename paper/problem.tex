...
However, it is not compositional in $\pe$: Consider the variable occurrence $y$;
...
prove that $\semusg{\pe}$ satisfies \Cref{thm:semusg-correct-live} by direct
structural induction on $\pe$ (in a way that would be useful to the proof).

Furthermore, \Cref{defn:deadness2} is not a property that is preserved under
...
value of $\pe$, say, $\Lam{y}{\pe'}$, \Cref{defn:deadness2} makes no assumption
