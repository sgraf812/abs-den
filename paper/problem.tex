...
%elaborate cardinality analyses~\cite{cardinality} and quantitative type
...
\[\begin{array}{c}
 ...
 \begin{array}{rrclcl}
  ...
  \text{Expressions}     &     \pe & ∈ & \Exp_0  & ::= & \px \mid \Lam{\px}{\pe} \mid \pe~\px \mid \Letsmall{\px}{\pe_1}{\pe_2} \\
 \end{array} \\
 \\[-0.5em]
 \begin{array}{rrclcl}
...
\[\begin{array}{rcl}
  ...
  \semusg{\Letsmall{\px}{\pe_1}{\pe_2}}_ρ& {}={} & \begin{letarray}
    ...
    \end{letarray}
\end{array}\]
\subcaption{Naïve usage analysis}
  \label{fig:usage}
\end{minipage}
\end{minipage}
}
...
computations. We could take inspiration in \cite{Milner:78} and work around the
issue by introducing a $\mathbf{wrong}$ denotation which is propagated strictly;
then we'd notice when we optimise a looping program into one that has a scoping
error.

Fortunately, our analysis actually behaves more benign than ``necessary'', so we
are hopeful that it adheres to a stricter notion of deadness, perhaps in terms
of a structural operational semantics such as Sestoft's Mark II machine
\cite{Sestoft:97} (an obvious pick for call-by-value would be a CESK machine
\cite{Felleisen:87} or a simpler derivative thereof). Machine states in
these transition systems are quadruples of control expression $\pe$, an
environment $ρ$ mapping lexically-scoped variables to their current heap
address, a heap $μ$ listing a closure for each adress, and a stack of
continuation frames $κ$. Then we'd be able to define as our notion of ``strong
deadness''
