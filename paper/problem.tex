...
discuss semantics of labels and data types in \Cref{sec:vanilla}.
...
\slpj{What is an ``emerging'' equation??
Perhaps: a solution to $\ScottD = [\ScottD \to_c \ScottD]_\bot$?}}
...
such as $\Usg$ that is inferred from context \slpj{What is this ``inferred from context'' bit.  This sound like a definition.}).
Let us now call
\[
...
Whenever $y$ is assumed \slpj{assumed?} to be live in $x$ (which is encoded as $\tr(x) ⊑
\tr(y)$ \slpj{What do you mean ``encoded''? Where do I see that?)},
...
has been accounted for at the call site \slpj{what do you mean ``accounted for at the call site''?},
