...
$\bot = 0 ⊏ d(x)$ \slpj{I can't even parse this!  Do you mean $d(x) \not= 0$?}
\sg{Yes, basically. This is a series of (in)equalities, not a definition (for
which I tend to use $\triangleq$).}
(where $\bot$ denotes the least element of a partial order
such as $\Usg$ that is inferred from context).
\slpj{What is this ``inferred from context'' bit.  This sound like a definition.}
\sg{Well, if you see a $u$ you immediately associate with it the partial order $0 ⊑ 1 ⊑ ω$
because $(\Usg,(⊑))$ is the only partial order that possibly could apply.
For $d ∈ \UsgD$ that would be the pointwise order. Simple, standard.}
Let us now call
\[
...
\slpj{Crumbs, this is a bit tricky.}
\sg{Yep. Necessarily so.}
...
\slpj{This is tricky! We \emph{pass in} an environment mapping free varaibles to something;
  and we \emph{return} a function mapping variables to something.}
\sg{Yes, just like how the $DmdType$ of free variables such as $x$ determine the
$DmdType$ of the expression $\pe$.
This stuff is tricky, and IMO necessarily so.}
\slpj{In Fig 1c you have $x \mapsto \bot$, but the Usg domain doesn't have $\bot$.  Or maybe
  you intend 0 and $\bot$ to be synonymous.  That's confusing -- at the very least you have to spell it out.
  Why is it right to map $x$ to 0 in the lambda rule?
  }
\sg{Actually, it has been spelled out already: $\Usg$ is a finite and a partial order, as its definition suggests.
So its $\bot$ element is of the course the least element $0$.}
...
Whenever $y$ is live in $x$ (which is encoded as $\tr(x) ⊑
\tr(y)$ in the $\mathbf{let}$ case of $\semusg{\wild}$),
the analysis will report that $(f~y)$ is live in $x$ \slpj{What does it mean to say `` $(f~y)$ is live in $x$''?},
...
accessible prose. \slpj{Need to point to evidence.  E.g. point to Appendix A?}
...
\begin{corollary}
  ...
  then $\px$ is dead in $\pe$.
\end{corollary}
\slpj{This one is much easier to understand.  Maybe just start with it? Or do you need
the generalisation for the proof?}
\begin{proof}
  ...
  \slpj{Again the compressed style makes it hard to read.   I see
    ``From the assumption $0 = \semusg{\pe}_{\tr_Δ}(\px)$ we know that $\tr_Δ(\px)(\px) = 1$'';
    but that that equality is nothing to do with the assumption!
    Better: Let us assume $\semusg{\pe}_{\tr_Δ}(\px) = 0$.  We know, by definition of $\tr_{\Delta}$ that $\tr_Δ(\px)(\px) = 1$.
...
of the domain (\eg, insufficiently specified input \slpj{What is ``insufficiently specified input''?}).
...
looping program into one that has a scoping error.  \slpj{Sorry don't get this, esp as the example is not looping.}
...
\Cref{thm:semusg-correct-live-simple}:
