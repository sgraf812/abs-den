...
In effect, \label{defn:deadness2} is an extensional condition of when a
...
\begin{lemma}[Never used implies dead]
...
Because it is so intuitively true, this part of proving an analysis correct \wrt
a transformation it enables is often neglected.
...
evaluates $x$ if $\pe_2$ never evaluates $x$ and $y$, but unfortunately
evaluation cardinality is not ``stable under reduction'':
The freshness condition is violated in the evaluation context
$(ρ[\px↦\pa],μ[\pa↦([],\Lam{z}{z})],κ)$. % TODO
