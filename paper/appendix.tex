...
\begin{proof}
  ...
  \dom(μ_i)$, so we can apply \Cref{thm:force-heap-bisimlar} to instead show
  \[
...
\begin{proof}[Proof of \Cref{thm:semusg-correct-live-3}]
  ...
  \begin{itemize}
    ...
    \item \textbf{Case $\pe = \Lam{\py}{\pe'}$}:
      ...
      by \Cref{thm:lazy-dead}, so the proof is a simple matter of applying the
    ...
    \item \textbf{Case $\pe = \pe'~\py$}:
      ...
      \]
      We will apply \Cref{thm:equiv-beta} and are left with two sub-goals:
      \begin{enumerate}
