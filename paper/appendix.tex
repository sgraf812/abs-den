...
\begin{lemma}[Congruence of $(\betastep)$]
...
\end{lemma}
\begin{proof}
  By applying \Cref{thm:lazy-bisimilar} twice and transitivity of $\wild ⊦ \wild \sim \wild$.
\end{proof}

\begin{definition}[Denotational deadness, operationally]
  \label{defn:addr-deadness}
  An address $\pa$ is \emph{dead} in a denotation $d$ if and only if,
  for all $μ_1 \approx μ_2$ such that $\adom(d) ⊆ \dom(μ_i) ∪ \{ \pa \}$
  and $d_1,d_2$, we have
  \[
...
\begin{lemma}
  ...
  If $\pa$ is dead in $\semevt{\pe}_{ρ[\px↦\deref(\pa)]}$ for any $ρ$ that $\pa$
  is dead in $\rng(ρ)$, then $\px$ is dead in $\pe$.
\end{lemma}
\begin{proof}
  ...
  \]
  For arbitrary $d_1,d_2$ (although we set
...
\begin{proof}[Proof of \Cref{thm:semusg-correct-live-3}]
  ...
  We fix $\pe$, $\px$ and $\tr$ such that $\tr(\px) \not⊑ \semusg{\pe}_{\tr}$.
  The goal is to show that $\px$ is dead in $\pe$.
  We do so by applying \Cref{thm:oper-dead-denot},
  so we show that an address $\pa$ is dead in $\semevt{\pe}_{ρ[\px↦\deref(\pa)]}$
  for any $ρ$ such that $\pa$ is dead in $ρ$.

  To show that, assume that $μ_1 \approx μ_2$, abbreviate
  $A \triangleq \dom(μ_1)$ and show
  \[
  ...
  \begin{itemize}
    \item \textbf{Case $\pe = \py$}: If $\px=\py$, then
      ...
      If $\px \not= \py$, then $\pa$ is dead in $\semevt{\py}_{ρ'}=ρ(\py)$.
    ...
    \item \textbf{Case $\pe = \pe'~\py$}:
      ...
      \]
      We will apply \Cref{thm:equiv-beta} here situation and are left with two
      sub-goals.
      \begin{enumerate}
