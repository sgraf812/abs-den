...
\begin{proof}[Proof of \Cref{thm:semusg-correct-live-3}]
  ...
  We generalise this statement to a whole set of addresses $\{\pa\}$.

  We assume that $μ_1 \approx μ_2$ such that $\dom(μ_1) ∪ \{\many{\pa}\} ⊦ \semevt{\pe}_{ρ[\many{\px↦\deref(\pa)}]}$ and
  abbreviate $A \triangleq \dom(μ_1)$. Then the goal is to show
  \[
    A ⊦ \semevt{\pe}_{ρ[\many{\px↦\deref(\pa)}]}(μ_1[\many{\pa↦d_1}]) \sim \semevt{\pe}_{ρ[\many{\px↦\deref(\pa)}]}(μ_2[\many{\pa↦d_2}])
  \]
  for any $\many{d_1},\many{d_2}$ and $ρ$ such that $\{\many{\pa}\}$ is dead in $\rng(ρ)$.
  ...
  \begin{itemize}
    ...
    \item \textbf{Case $\pe = \Lam{\py}{\pe'}$}:
      We abbreviate
      $μ_i' \triangleq μ_i[\many{\pa↦d_i}]$, $ρ' \triangleq ρ[\many{\px↦\deref(\pa)}]$
      and have to show
      \[
      ...
      \sg{Idea: $d \equiv d$, so $A ⊦ d(μ_1) \sim d(μ_2)$ and both heaps force appropriately.}
