...
\begin{proof}[Proof of \Cref{thm:semusg-correct-live-3}]
  ...
  \begin{itemize}
    ...
    \item \textbf{Case $\pe\triangleq\Lam{\py}{\pe'}$}:
      ...
      This follows by applying congruence rules for $\Events$, $\Traces{}$ and
      $\EventD$ once we can show the crucial property
      $d_1 \overset{\pa}{\sim}_{\EventD} d_2$
      for $d_i \triangleq \semevt{\pe'}_{ρ[\px↦\deref(\pa)][\py↦d_i^\later]}$.

      If $\px=\py$ then $d_i = \semevt{\pe'}_{ρ[\py↦d_i^\later]}$ and nothing
      references $\pa$, so the equivalence follows by induction on $\pe'$.

      Otherwise, we can rearrange the environments to
      $d_i = \semevt{\pe'}_{ρ[\py↦d_i^\later][\px↦\deref(\pa)]}$.
      Now, $d_i^\later
      and hence it suffices to show that $\px$ is dead in assuming we can show that $\px$ is dead in $\pe'$,
      we can see that $\pa$ is dead in
      $d_i \triangleq \semevt{\pe'}_{ρ[\px↦\deref(\pa)][\py↦d_i^\later]}$ by definition of
      deadness and then easily conclude the proof
