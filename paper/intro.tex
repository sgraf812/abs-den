...
\item Re-imagine and re-implement the analysis as an abstraction of the operational semantics.
  ...
  that drives the entire analysis into lower level detail. \slpj{not a great critique}
\end{itemize}
...
\item We describe a new \emph{trace-based denotational semantics for lambda calculus},
  ...
  trace that we can later elaborate with as much operational detail as we please (\Cref{sec:vanilla}).
    ...
    mathematical function. \slpj{Seems out of place here.  Need to say more, or less.}
    Our semantics is distinct from similar ones for call-by-name or
    call-by-value \slpj{what does that mean?}.
    We prove it adequate \wrt a standard operational semantics.  \slpj{This is the ``hard proof''??  We do it once for all.}
  ...
  \item We employ the eventful semantics as a collecting semantics and derive
    ...
    \sg{TODO: Currently we only attempt a direct correctness proof}
    \slpj{contributions seem to peter out...}
\end{itemize}
...
contexts without the need for a control-flow graph abstraction.\slpj{fwd ref to the section that elaborates this claim}
