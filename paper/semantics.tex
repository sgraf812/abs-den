...
\[\begin{array}{c}
 \begin{array}{rcl}
  ...
  \\[-0.5em]
  val(\pv,v)(μ) & = & (\pv,μ) \cons \sing{((\pv,v),μ)} \\
  \\[-0.5em]
  update(τ) & = & \begin{cases}
      τ :: \sing{((\pv,v),μ[\pa ↦ val(\pv,v)])} & \text{$τ = ... \cons \sing{((\pv,v),μ)}$} \\
      τ & \text{otherwise} \\
    \end{cases} \\
  \\[-0.5em]
  deref(\pa)(μ) & = & update(μ(\pa)(μ)) \\
  \\[-0.5em]
  ...
  \end{cases} \\
  \\[-0.5em]
  \semst{\px}_ρ(μ) & = & \ternary{\px ∈ \dom(ρ)}{(\px,μ) \cons ρ(\px)(μ)}{\sing{(\px,μ)}} \\
  \\[-0.5em]
...
\[\begin{array}{c}
 \begin{array}{rcl}
  ...
  \end{cases} \\
  \\[-0.5em]
  \seminf{\px}_ρ(μ) & = & \ternary{\px ∈ \dom(ρ)}{(\px,μ) \cons ρ(\px)(μ)}{\sing{(\px,μ)}} \\
  \\[-0.5em]
  \seminf{\Lam{\px}{\pe}}_ρ & = & val(\Lam{\px}{\pe},\FunV(\fn{d}{\seminf{\pe}_{ρ[\px↦d]}})) \\
  \\[-0.5em]
  \seminf{\pe~\px}_ρ(μ) & = & \begin{cases}
      \pe~\px \act{\LookupA(\pa)} apply(\seminf{\pe}_ρ(μ),ρ(\px)) & \px ∈ \dom(ρ) \\
      \sing{(\pe~\px,μ)} & \text{otherwise} \\
    \end{cases} \\
  \\[-0.5em]
  \seminf{\Let{\px}{\pe_1}{\pe_2}}(μ) & = & \begin{letarray}
    \text{let} & ρ' = ρ[\px ↦ deref(\pa)] \quad \text{where $\pa \not∈ \dom(μ)$} \\
               & μ' = μ[\pa ↦ \seminf{\pe_1}_{ρ'}] \\
    \text{in}  & (\Let{\px}{\pe_1}{\pe_2},μ) \cons \seminf{\pe_2}_{ρ'}(\pe_2,μ') \\
  \end{letarray} \\
 \end{array} \\
\end{array}\]
\caption{Structural call-by-need stateful trace semantics $\semst{-}$}
