...
\[\begin{array}{c}
 \begin{array}{rcl}
  ...
  \\[-0.5em]
  \makestuck{\goodend{σ}} & = & \stuckend{σ} \\
  \makestuck{\stuckend{σ}} & = & \stuckend{σ} \\
  \makestuck{σ \cons τ} & = & σ \cons \makestuck{τ} \\
  \\[-0.5em]
  val(\pv,v)(μ) & = & (\pv,μ) \cons \goodend{(v,μ)} \\
  \\[-0.5em]
  update(\pa,τ) & = & \begin{cases}
      τ :: \goodend{(v,μ[\pa ↦ val(\pv,v)])} & \text{$τ = ... \cons (\pv,\wild) \cons \goodend{(v,μ)}$} \\
      \makestuck{τ} & \text{otherwise} \\
    \end{cases} \\
  \\[-0.5em]
  deref(\pa)(μ) & = & update(\pa,μ(\pa)(μ)) \\
  \\[-0.5em]
  ifz(τ,d_t,d_e) & = & \begin{cases}
    τ :: d_t(μ) & \text{if $τ =... \cons \goodend{((\wild,\NatV(0)),μ)}$} \\
    τ :: d_e(μ) & \text{if $τ =... \cons \goodend{((\wild,\NatV(n)),μ)}, n \not= 0$} \\
    \makestuck{τ} & \text{otherwise} \\
  \end{cases} \\
  \\[-0.5em]
  apply(τ,d) & = & \begin{cases}
    τ :: f(d)(μ) & \text{if $τ =... \cons \goodend{((\wild,\FunV(f)),μ)}$} \\
    \makestuck{τ} & \text{otherwise} \\
  \end{cases} \\
  \\[-0.5em]
  ...
  \\[-0.5em]
  \semst{\pe@(\Ifz{\pe_b}{\pe_t}{\pe_e})}_ρ(μ) & = & \begin{cases}
      (\pe,μ) \cons ifz(\semst{\pe_b}_ρ(μ,\mathbf{if} \pushF κ),\semst{\pe_t}_ρ,\semst{\pe_e}_ρ) & \px ∈ \dom(ρ) \\
      \stuckend{(\pe,μ)} & \text{otherwise} \\
    \end{cases} \\
  \\[-0.5em]
  \semst{\pe~\px}_ρ(μ) & = & \begin{cases}
      (\pe~\px,μ) \cons apply(\semst{\pe}_ρ(μ,\ApplyF \pushF κ),ρ(\px)) & \px ∈ \dom(ρ) \\
      \stuckend{(\pe~\px,μ)} & \text{otherwise} \\
    \end{cases} \\
  \\[-0.5em]
  \semst{\Let{\px}{\pe_1}{\pe_2}}(μ) & = & \begin{letarray}
...
\midrule
\BindT & (\Let{\px}{\pe_1}{\pe_2},ρ,μ,κ) & \smallstep & (\pe_2,ρ',μ[\pa↦(\pe_1,ρ',d_1)], κ) & \pa \not∈ \dom(μ),\ ρ'\! = ρ[\px↦\pa] \\
...
\sg{\citep{LeroyGrall:09} show in Lemma 10 that there exists a maximal trace for
...
%  \emph{fully abstract}~\cite{Plotkin:77}, meaning that
%
