...
\begin{itemize}
  \item \textbf{Case $\px$}:
    ...
    and this must be the final state $σ_n$, because it remains a return state
    ...
    its second argument which makes another step to the final state of $τ$,
  ...
  \item \textbf{Case $\pe~\px$}:
    ...
    That in turn means that the value in $\ctrl(σ_m)$ must a lambda
  ...
  \item \textbf{Case $\Let{\px}{\pe_1}{\pe_2}$}:
    ...
    = μ_1[\pa↦(ρ_2,\pe_1)]$.
    Since the stack does not grow, maximality from $σ_{i+1})_{i∈\overline{n-1}}$
    must be maximal, too.
...
strong version of adequacy for $\semst{\wild}$:
...
in the sense of \citep[]{Cousot:21}.
...
\sg{Why not omit labelled expressions, then?}
