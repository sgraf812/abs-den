...
\[\begin{array}{c}
 \begin{array}{rcl}
  ...
  \\[-0.5em]
  val(\pv,v)(μ,κ) & = & (\pv,μ,κ) \cons \sing{((\pv,v),μ,κ)} \\
  \\[-0.5em]
  ...
  \end{cases} \\
  \\[-0.5em]
  \semst{\px}_ρ(μ,κ) & = & \ternary{\px ∈ \dom(ρ)}{(\px,μ,κ) \cons ρ(\px)(μ,\UpdateF \pushF κ)}{\sing{(\px,μ,κ)}} \\
  \\[-0.5em]
  \semst{\Lit{n}}_ρ(μ,κ) & = & val(\Lit{n},\NatV(n)) \\
  \\[-0.5em]
  \semst{\Lam{\px}{\pe}}_ρ & = & val(\Lam{\px}{\pe},\FunV(\fn{d}{\semst{\pe}_{ρ[\px↦d]}})) \\
  \\[-0.5em]
  \semst{\Ifz{\px}{\pe_t}{\pe_e}}_ρ(μ,κ) & = & \begin{cases}
      (\Ifz{\px}{\pe_t}{\pe_e},μ,κ) \cons ifz(ρ(x)(μ,\mathbf{if} \pushF κ),\semst{\pe_t}_ρ,\semst{\pe_e}_ρ) & \px ∈ \dom(ρ) \\
      \sing{(\Ifz{\px}{\pe_t}{\pe_e},μ,κ)} & \text{otherwise} \\
    \end{cases} \\
  \\[-0.5em]
...
\[\begin{array}{c}
 \begin{array}{rcl}
  ...
  \\[-0.5em]
  update(\pa,τ) & = & \begin{cases}
      τ :: \sing{((\pv,v),μ[\pa ↦ val(\pv,v)])} & \text{$τ = ... \cons \sing{((\pv,v),μ)}$} \\
      τ & \text{otherwise} \\
    \end{cases} \\
  \\[-0.5em]
  deref(\pa)(\px,μ) & = & \begin{letarray}
      \text{let} & (\pe,d) = μ(\pa) \\
      \text{in} & (\px,μ) \cons update(\pa,d(\pe,μ)) \\
    \end{letarray} \\
  \\[-0.5em]
  apply(τ,d) & = & \begin{cases}
    τ :: f(d)(μ) & \text{if $τ =... \cons \sing{((\wild,\FunV(f)),μ)}$} \\
    τ & \text{otherwise} \\
  \end{cases} \\
  \\[-0.5em]
  ...
  \semst{\pe~\px}_ρ(μ) & = & \begin{cases}
      ...
      \sing{(\pe~\px,μ)} & \text{otherwise} \\
    \end{cases} \\
  \\[-0.5em]
  \semst{\Let{\px}{\pe_1}{\pe_2}}(μ) & = & \begin{letarray}
    \text{let} & ρ' = ρ[\px ↦ deref(\pa)] \quad \text{where $\pa \not∈ \dom(μ)$} \\
               & μ' = μ[\pa ↦ (\pe_1,\semst{\pe_1}_{ρ'})] \\
    \text{in}  & (\Let{\px}{\pe_1}{\pe_2},μ) \cons \semst{\pe_2}_{ρ'}(\pe_2,μ') \\
  \end{letarray} \\
 \end{array} \\
\end{array}\]
\caption{Structural call-by-need stateful trace semantics $\semst{-}$}
