...
\begin{figure}
\[\begin{array}{rcl}
  α_\Environments([\many{\px ↦ \pa}]) & = & [\many{|x| ↦ |fetch a|}] \\
  ...
  α_\Events(σ) & = & \begin{cases}
    |Let1| & σ = (\Let{\wild}{\wild}{\wild},\wild,\wild,\wild) \\
    |App1| & σ = (\wild~\px,\wild,\wild,\wild) \\
    |Case1| & σ = (\Case{\wild}{\wild},\wild,\wild, \wild)\\
    |Lookup x| & σ = (\px,\wild,\wild,\wild) \\
...
In a slight abuse of notation, we write |eval e tr tm| to mean
|case eval e tr of ByNeed m -> m tm|, as if |ByNeed| was a type synonym.
...
\begin{itemize}
  \item \textbf{Case $\px$}:
    ...
    We can infer that |tm a = memo a (eval e' tr')| from the
    definition of $α_\Heaps$, so
    \begin{spec}
      tm a tm = eval e tr' tm >>= Step Update . ByNeed . StateT . upd
        where upd (val, tm') = Ret (val, ext tm' a (memo a (return val)))
    \end{spec}
    ...
    in |Ret Stuck| then |>>=| will return |tl|, indicating
    ...
    balanced.
    Likewise, by the second argument of |>>=|, we do a |Step Update| on
    |Ret (val,tmm)|, updating the heap to
    \begin{spec}
      Ret (val,ext tmm a (memo a (return val))) = Ret (val,ext tmm a (memo a (eval v trm))) = αState σn
    \end{spec}
  ...
  \item \textbf{Case $\pe~\px$}:
    ...
    \idiom{\sem{\pe}_{ρ}(\tm)}$ returns and let $σ_m$ be LK state at the
    ...
    $\Lam{\py}{\pe'}$, hence $τ^{\later}$ ends in $\goodend{\FunV(\fn{\pa}{\idiom{\sem{\pe'}_{ρ_m[\py ↦ \pa]}}}), \tm_m} = σ_\VanV(σ_m)$
    ...
    $τ_2^{\later} \triangleq \idiom{\sem{\pe'}_{ρ_m[\py↦\pa]}(\tm_m)}$
    to get the equality
...
\begin{theoremrep}[Total adequacy of |eval|]
  ...
  Let |τ triangleq fmap fst (runByNeed (eval e emp)) :: T (Value _)|.
  \begin{itemize}
    \item
...
\begin{proof}
  ...
  The correctness of |Event|s emitted follows directly from the structure of
  $α_{\STraces}$.
  \begin{itemize}
    \item[$\Rightarrow$]
      \begin{itemize}
