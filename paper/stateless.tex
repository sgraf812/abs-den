...
\[\begin{array}{c}
 ...
 \begin{array}{rrclcl}
  ...
  \text{Initialisation Traces}   & τ^+      & ∈ & \Traces^+             & ::=        & \goodend{a} \mid a \cons τ^+  \\
  \text{Finite and infinite Traces} & τ & ∈ & \Traces^{+\infty} & ::= & \goodend{a} \mid \stuckend{} \mid a \cons τ^{\later}  \\
  \text{Delayed Trace} & τ^{\later} & ∈ & \later \Traces^{+\infty} &  &   \\
  \text{Trace list syntax} &            &   & [a_1,a_2,...,a_n]_\checkmark & = & a_1 \cons \idiom{a_2 \cons \idiom{ ... \cons \idiom{\goodend{a_n}}}}  \\
  \\
...
\[\begin{array}{c}
 ...
 \begin{array}{cc}
  ...
  \begin{array}{rcl}
   \\[-0.5em]
   (d_1 \fcomp d_2^{\later})(τ_i^+)   & = & d_1(τ_i^+) \concat \idiom{d_2^{\later}(τ_i^+ \concat d_1(τ_i^+))} \\
   \\[-0.5em]
   \tell{a}(τ_i^+) & = & [a]_\checkmark \\
  \end{array} \\
 \end{array} \\
 \\[-0.5em]
 \begin{array}{rcl}
  ...
  \\[-0.5em]
  β(f)(τ_i^+) & = & \begin{cases}
      \idiom{f(v)(τ_i^+)} & \text{$τ_i^+ = [..., \ValueA(v), \many{\UpdateA(\wild)}]_\checkmark$ and $v ∈ \dom(f)$} \\
      \idiom{\stuckend{}} & \text{otherwise} \\
    \end{cases} \\
  \\[-0.5em]
  μ(τ_i^+)(\pa) & = & \begin{cases}
    \tell{\ValueA(v)} & \text{if $τ_i^+ = [..., \ValueA(v), \many{\UpdateA(\wild),}\, \UpdateA(\pa),...]_\checkmark$} \\
    d & \text{if $τ_i^+ = [..., \BindA(\px,\pa↦d), ...]_{\checkmark}$} \\
  \end{cases}  \\
  \\[-0.5em]
  \deref(\pa)(τ_i^+)   & = & (\tell{\LookupA(\pa)} \fcomp μ(τ_i^+)(\pa) \fcomp \tell{\UpdateA(\pa)})(τ_i^+) \\
 ...
 \begin{array}{rcl}
  ...
  \seminf{\Let{\px}{\pe_1}{\pe_2}}_ρ(τ_i^+) & = &
    \begin{letarray}
      ...
      \text{in}  & (\tell{\BindA(\px,\pa↦\seminf{\pe_1}_{ρ'})} \fcomp \seminf{\pe_2}_{ρ'})(τ_i^+)
    \end{letarray} \\
  \\[-0.5em]
