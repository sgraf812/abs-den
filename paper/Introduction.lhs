...
In this work, we show how to break down this complexity.
...
machine's run on that term.
We apply this approach to call-by-need, yielding the first denotational
semantics for by-need with that property.
...
an element of the analysis domain.
...
\item \Cref{sec:interp} walks through the definition of our generic
  ...
  The exposition assumes a by-name evaluation strategy (\Cref{sec:dna}),
  but \Cref{sec:by-need-instance} shows how easily
  the interpreter adjusts to by-need, each instance producing abstract machine traces.
\item \Cref{sec:adequacy} proves that the traces generated by the by-need
  ...
  standard lazy Krivine (LK) machine~\citep{Sestoft:97}.
  Thus a concrete denotational interpreter reproduces the abstract machine step
  for step, and can serve as ground truth for operational behaviour.
\item
...
\item
  Thus we have a \emph{dynamic semantics} as well as a \emph{static analysis}, each obtained by
  instantiating the same denotational interpreter.
  In \Cref{sec:soundness}, we prove that the static analysis approximates the
  behaviors of the dynamic semantics.
  ...
  operational semantics is considered ground truth.
\item
  \Cref{sec:mechanisation} mechanises the by-need development in Lean on top of the
  Iris separation logic~\citep{Jung:18}: the generic interpreter, its productivity, its
  adequacy \wrt the LK machine, and the soundness of usage analysis, culminating in a
