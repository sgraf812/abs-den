...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item \textbf{Case $\pe = \Let{\py}{\pe_1'}{\pe_2'}$}:
      Unfolding $\ctx_D$ and $\semusg{\Lam{\py}{\pe'}}_{\tr}$ once, we have
      \[\begin{array}{l}
        u_l = \ctx_{D}(\usg_\EventD(\semevt{\pe_2'}_{ρ_1[\py↦\pa']})(\tm[\pa'↦\usg_\EventD(\semevt{\pe_1'}_{ρ_1[\py↦\pa']}))(\pa) \} \\
        u_r = \semusg{\pe_2'}_{\tr'}(\px) \\
      \end{array}\]
      where $\tr' \triangleq \fix(\fn{\tr'}{\tr ⊔ [\px ↦ [\px↦1]+\semusg{\pe_1'}_{\tr'}]})$.

      When $\semusg{\pe_1'}_{\tr'}(\px) = 0$, we need to add $\pa'$ to $D$.
      Before we can apply the induction hypothesis, we must verify that
      \Cref{eqn:semusg-rho} holds for $\py$:
      \begin{DispWithArrows*}
            & \tr_Δ(\py)(\px) + \ctx_D(\tm[\pa'↦...](ρ_1[\py↦\pa'](\py))(\tm))(\pa) \Arrow{definition of $\tr_Δ$, $\px\not=\py$} \\
        ={} & 0 + \ctx_D(\tm(\pa')(\tm))(\pa) \Arrow{$\pa'∈D$} \\
        ={} & 0 + 0 \Arrow{simplify} \\
        ={} & 0  \Arrow{simplify} \\
        ={} & \tr[\py↦\bot]
