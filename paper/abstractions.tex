...
\end{lemmarep}
\begin{proof}
We fix $d$, abbreviate $\pe_{\mathsf{let}} \triangleq \Let{\py}{\pe_1}{\pe_2}$ and show
\[
  \noalias(\px,\pa,ρ_\cong) \wedge \dead(\pa,μ_\cong) ⊦ \semevt{\pe_{\mathsf{let}}}_{ρ_\cong[\px↦\pa]}(μ_\cong) \cong \semevt{\pe_{\mathsf{let}}}_{ρ_\cong[\px↦\pa]}(μ_\cong[\pa↦d])
\]
we unfold the let case, allocate a fresh $\pa'$,
abbreviate $ρ' \triangleq ρ_\cong[\py↦\pa']$,
$d_r \triangleq \memo(\pa',\semevt{\pe_1}_{ρ'})$
and simplify to
\[
  \noalias(\px,\pa,ρ_\cong) \wedge \dead(\pa,μ_\cong) ⊦ \semevt{\pe_2}_{ρ'}(μ_\cong[\pa'↦d_r]) \cong \semevt{\pe_2}_{ρ'}(μ_\cong[\pa↦d,\pa'↦d_r])
\]
$\px$ is dead in $\pe_1$ per assumption.
It is also $\noalias(\px,ρ')$ in the antecedent and both
...
\begin{itemize}
  ...
  \item \textbf{Case $\pe = \pe'~\py$}:
    ...
    To show that $\px$ is dead in $\pe'~\py$, the goal is to show
    \[
      \noalias(\px,\pa,ρ_\cong) \wedge \dead(\pa,μ_\cong) ⊦ \semevt{\pe'~\py}_{ρ_\cong}(μ_\cong) \cong \semevt{\pe'~\py}_{ρ_\cong}(μ_\cong[\pa↦d]),
    \]
    where we assume that $ρ(\px) = \pa$ and $\pa' \triangleq ρ_\cong(\py) \not= \pa$.

    The interesting case is when $\pe'$ does not diverge and ultimately reduces
    to a lambda; otherwise, $\betastep$ will get stuck.
