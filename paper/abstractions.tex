...
\begin{definition}[Denotational deadness, lazily]
  ...
  \[
    (ρ_\cong=ρ) \wedge \later(\dead(\pa,μ_\cong)) ⊦ \semevt{\pe}_{ρ_\cong}(μ_\cong) \cong \semevt{\pe}_{ρ_\cong}(μ_\cong[\pa↦d])
  \]
...
\[
  \pa \not∈ \rng(ρ_\cong) \wedge \later(\dead(\pa,μ_\cong)) ⊦ \semevt{\pe_{\mathsf{let}}}_{ρ_\cong[\px↦\pa]}(μ_\cong) \cong \semevt{\pe_{\mathsf{let}}}_{ρ_\cong[\px↦\pa]}(μ_\cong[\pa↦d])
\]
...
and simplify to
\[
  \pa \not∈ \rng(ρ_\cong) \wedge \later(\dead(\pa,μ_\cong)) ⊦ \semevt{\pe_2}_{ρ'}(μ_\cong[\pa'↦d_r]) \cong \semevt{\pe_2}_{ρ'}(μ_\cong[\pa↦d,\pa'↦d_r])
\]
$\px$ is dead in $\pe_1$ per assumption.
It is also $\pa \not∈ \rng(ρ) ∪ \{\pa'\}$ and both facts together imply that
$\pa$ is dead in $\semevt{\pe_1}_{ρ'}$; similarly for $\pe_2$.
That in turn implies that $\dead(\pa,μ_\cong[\pa↦d,\pa'↦d_r])$, for any $d$.
