...
\begin{definition}[Denotational deadness, lazily]
  ...
  any $\pa$ is dead in $\semevt{\pe}_{ρ[\px↦\deref(\pa)]})$ for any $ρ$ such
  that $\pa$ is dead in $\rng(ρ)$.
...
\end{lemma}
\begin{proof}
  Assume that $\pa$ is dead in $\semevt{\pe}_{ρ[\px↦\deref(\pa)]})$ for any
  $ρ$ such that $\pa$ is dead in $\rng(ρ)$.
  To show that
  \[
...
\begin{definition}[Usage cardinality]
  ...
  for $\pa$ dead in $\rng(ρ)$, $\semevt{\pe}_{ρ[\px↦\deref(\pa)]}$ evaluates
  $\pa$ at most $u$ times.
\end{definition}
...
\begin{proof}
  ...
  \[
    \dom(μ_i) ⊦_0 \semevt{\pe_2}_{ρ[\px↦\deref(\pa)]}(μ_1[\pa↦d_1]) \lesssim\!\gtrsim \semevt{\pe_2}_{ρ[\px↦\derefn(\pa)]}(μ_2[\pa↦d_2])
  \]
  where $\pa \not∈\dom(μ_i)$,
  $\dom(μ_i) ∪ \{\pa\} ⊦ \semevt{\pe_2}_{ρ[\px↦\deref(\pa)]}$,
  $\dom(μ_i) ∪ \{\pa\} ⊦ \semevt{\pe_2}_{ρ[\px↦\derefn(\pa)]}$,
  $μ_1 \lessapprox\!\gtrapprox μ_2$, and $\pa$ is dead in both
  $d_1 \triangleq \semevt{\pe_1}_{ρ[\px↦\deref(\pa)]}$ and
  $d_2 \triangleq \semevt{\pe_1}_{ρ[\px↦\derefn(\pa)]}$. \\
  (NB: $d_2$ uses $\derefn(\pa)$, which is at odds with the definition of
  deadness.
  It should not be hard to prove that if $\pa$ is dead in $d_1$, it is also dead
  in $d_2$, by induction over $\pe_1$.)

  By induction on $\pe_2$.
  \begin{itemize}
    \item \textbf{Case $\pe_2 = \py$}: If $\px=\py$, the goal reduces to
      \[
        \dom(μ_i) ⊦_0 \deref(\pa)(μ_1[\pa↦d_1]) \lesssim\!\gtrsim \derefn(\pa)(μ_2[\pa↦d_2])
      \]
      Since the tick makes up for the additional step that heap update incurs, it suffices
      to show that
      \[
        \dom(μ_i) ⊦_0 d_1(μ_1[\pa↦d_1]) \lesssim\!\gtrsim d_2(μ_2[\pa↦d_2])
      \]
      and since $\pa$ is dead in $d_i$, we have
      \[
        \dom(μ_i) ⊦_0 d_1(μ_1[\pa↦d_2]) \lesssim\!\gtrsim d_1(μ_1[\pa↦d_2]) \lesssim\!\gtrsim d_2(μ_2[\pa↦d_2])
      \]
      and the latter equality is another small auxiliary lemma (the entry for
      $\px$ is irrelevant).

      Since $\ctx_\LookTraces(\usg_\Traces(\deref(\pa)(μ_1[\pa↦d_1])))(\pa)⊑1$
      and $\deref(\pa)(μ_1[\pa↦d_1])$ emits a $\LookupA(\pa)$ event as its
      first action, and because $1 + u = 1$ implies $u = 0$, we must have
      $\ctx_\LookTraces(\usg_\Traces(d_1(μ_1[\pa↦d_1]))) = 0$.

      Now let $f_i$, $μ_i'$ such that $\bigstep{d_i}{μ_i[\pa↦d_i]}{\FunV(f_i)}{μ_i'}$.


    then
      $\semevt{\px}_{ρ[\px↦\deref(\pa)]}(μ_1[\pa↦d_1]) = \deref(\pa)(μ_1[\pa↦d_1])$ and
      $\semevt{\px}_{ρ[\px↦\derefn(\pa)]}(μ_2[\pa↦d_2]) = \derefn(\pa)(μ_2[\pa↦d_2])$.
