...
%\Cref{thm:ok-ctx-dead} to show the goal.
%
%So fix $ρ,μ,\pa$ such that $\ok(\px,\pa,ρ,μ,A)$
%and unfold the definition of $\semevt{\hole~\py}(S)_ρ(μ)$ and $\apply$ to
%%TODO
%see the call $S(ρ[\py↦])(μ)$.
%Since $\py \not= \px$ and $\pa' \not∈ \dom(μ)$, we have $\noalias(\px,\pa,ρ[\py↦\pa'])$.
%Furthermore, since $\px$ is dead in $\pe_1$, we have that
%$\pa$ is dead in $\semevt{\pe_1}_{ρ[\py↦\pa']}$; hence
%$\dead(\pa,μ[\pa'↦\memo(\pa',\semevt{\pe_1}_{ρ[\py↦\pa']})])$.
  ...
  \item \textbf{Case $\pe = \Let{\py}{\pe_1'}{\pe_2'}$}:
    ...
    \begin{itemize}
      \item \textbf{Case $\tr'(\px) ⊑ \semusg{\pe_1'}_{\tr'}$}: Then
        ...
        %TODO: Not enough!! need deadness, not irrelevance
        \begin{DispWithArrows*}
                   & \Let{\px}{\pe_1}{\Let{\py}{\pe_1'}{\pe_2'}} \Arrow{$\dead(\py,\pe_2')$, \Cref{thm:dead-irrelevant}} \\
          \equiv{} & \Let{\px}{\pe_1}{\Let{\py}{\Lam{z}{z}}{\pe_2'}} \Arrow{$\px \not∈ \fv(\Lam{z}{z})$, let floating} \\
          \equiv{} & \Let{\py}{\Lam{z}{z}}{\Let{\px}{\pe_1}{\pe_2'}} \Arrow{$\dead(\px,\pe_2')$, \Cref{thm:dead-irrelevant}} \\
          \equiv{} & \Let{\py}{\Lam{z}{z}}{\Let{\px}{\pe_2}{\pe_2'}} \Arrow{$\px \not∈ \fv(\Lam{z}{z})$, let floating} \\
