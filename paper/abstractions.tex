...
\begin{proof}
  ...
  We abbreviate
  $u_l \triangleq \ctx_{D}(\usg_\EventD(\semevt{\pe}_ρ)(\tm))(ρ(\px))$,
  ...
  \begin{itemize}
    \item \textbf{Case $\pe = \py$}:
      ...
      Otherwise, when $\px = \py$, we have $u_l = 1 + \ctx_{D}(\tm(ρ_1(\px))(\tm))(\pa)$
      and $u_r = \tr(\px)(\px) ⊒ \tr_Δ(\px)(\px) + \ctx_D(\tm(ρ_1(\px))(\tm))(\pa)$
      by \Cref{eqn:semusg-rho}.
      Clearly, $\tr_Δ(\px)(\px) = 1$, so we combine to get $u_r ⊒ u_l$, the
      goal.

      When $\px \not= \py$, then $ρ(\px) \not= ρ(\py)$ and we have $u_l =
      0 + \ctx_{D}(\tm(ρ_1(\py))(\tm))(\pa)$ and $u_r = \tr(\py)(\px) ⊒
      \tr_Δ(\py)(\px) + \ctx_D(\tm(ρ_1(\py))(\tm))(\pa)$ by the induction
      hypothesis. Clearly, $\tr_Δ(\py)(\px) = 0$, so we combine to get $u_r ⊒
      u_l$, the goal.

    \item \textbf{Case $\pe = \Lam{\py}{\pe'}$}:
      We have
      \[\begin{array}{l}
        u_l = \ctx_{D}(\usg_\EventD(\semevt{\pe'}_{ρ_1[\py↦\text{some address from $D$}]})(\tm))(\pa) \\
        u_r = \semusg{\pe'}_{\tr[\py↦\bot]}(\px) \\
      \end{array}\]

    \item \textbf{Case $\pe = \pe'~\py$}:
      If $\tr(\py)(\px) \not= 0$, then $u_r = ω$, the top element, and the goal follows.

      Otherwise, $\tr(\py)(\px) = 0$ and
  \end{itemize}
\end{proof}
