...
\begin{definition}[Deadness]
  ...
  for all expressions $\pe_1, \pe_2$. Otherwise, $\px$ is \emph{live}.
\end{definition}
...
So if $x$ is dead in $\pe_2$, we can justify the following rewrite:
\[
...
\begin{toappendix}
\begin{definition}[Contextual deadness]
  \label{defn:ctx-dead}
  A variable is \emph{dead} in a context $\pE$ if and only if $\px$ is dead in
  $\pE[\pe]$ whenever $\px$ is dead in an expression $\pe$.
\end{definition}
\end{toappendix}
...
\[\begin{array}{c}
 ...
 \\[-0.5em]
 \inferrule*[right=]
    {\forall \pE.\ \good(\px,\pE) \Longrightarrow \dead(\px,\pE[\pe])}
    {\good(\px,\pe)}
 \\
 \\[-0.5em]
 \inferrule*[right=]
 ...
 \quad
 \inferrule*[right=]
    {\good(\px,\pE) \quad \later(\good(\px,\pe))}
    {\good(\px,\Let{\py}{\pe}{\pE})}
 \\
\end{array}\]
\caption{Correctness relation for \Cref{thm:semusg-correct-live-3}}
  \label{fig:semusg-correct-live}
\end{figure}

\begin{lemmarep}
  \label{thm:var-good}
  If $\px \not= \py$, then $\good(\px,\py)$.
\end{lemmarep}
\begin{proof}
...
\begin{lemmarep}
  \label{thm:var-good}
  If $\px \not∈ \fv(\pe)$, then $\good(\px,\pe)$.
\end{lemmarep}
\begin{proof}
  Let us fix $\pE$ such that $\good(\px,\pE)$ and show that $\px$ is dead in
  $\pE[\pe]$, by induction on $\good(\px,\pE)$.
\begin{itemize}
  \item \textbf{Case $\pE = \hole$}:
    Simple proof by induction on $\pe$,
    exploiting that $\px$ does not occur in $\pe$.
  \item \textbf{Case $\pE = \pE'~\py$}:
    Then $\good(\px,\pE')$, $\px \not= \py$ and $\px$ is dead in $\pE'[\pe]$.
    Witness
    \begin{DispWithArrows*}
               & \Let{\px}{\pe_1}{\pE'[\pe]~\py} \Arrow{$\px \not∈ \fv(\hole~\py)$, let floating} \\
      \equiv{} & \Let{\px}{\pe_1}{\pE'[\pe]}~\py \Arrow{$\dead(\px,\pE'[\pe])$} \\
      \equiv{} & \Let{\px}{\pe_2}{\pE'[\pe]}~\py \Arrow{$\px \not∈ \fv(\hole~\py)$, let floating} \\
      \equiv{} & \Let{\px}{\pe_2}{\pE'[\pe]~\py}
    \end{DispWithArrows*}
  \item \textbf{Case $\pE = \Let{\py}{\pe}{\pE'}$}:
    Then $\good(\px,\pE)$ and $\later(\good(\px,\pe))$.
\end{itemize}
\end{proof}
...
\begin{proof}
  Let us fix $\pE$ such that $\good(\px,\pE)$ and show that $\px$ is dead in
  $\pE[\py]$, by induction on $\pE$.
\begin{itemize}
  \item \textbf{Case $\pE = \hole$}: Immediate, since $\px$ is dead in $\pE[\py]$.
  \item \textbf{Case $\pE = \pE'~\pz$, $\px \not= \pz$}: Immediate, since $\px$ is dead in $\pE[\py]$.
...
\end{theoremrep}
\begin{proof}
The setting is similar to \Cref{thm:semusg-correct-live}; we generalise the
induction hypothesis to the predicate
\[
  \correct(\px,\pe,\tr) \triangleq \tr(\px) \not⊑ \semusg{\pe}_\tr \Longrightarrow \good(\px,\pe),
\]
where $\good$ is a predicate defined on expressions and contexts given in
\Cref{fig:semusg-correct-live}.
The goal follows by instantiating $\correct(\px,\pe,\tr_Δ)(\hole)$.

We proceed by induction on $\pe$.
Stuck cases are boring and hence omitted.
\begin{itemize}
  \item \textbf{Case $\pe = \py$}:
    If $\py = \px$, then $\semusg{\py}_{\tr} = \tr(\py)$,
    contradicting the inequality.
    If $\py \not= \px$, then $\px$ is good in $\py$ by \Cref{thm:var-good}.
  ...
  \item \textbf{Case $\pe = \Let{\py}{\pe_1'}{\pe_2'}$}:
    ...
    \begin{itemize}
      \item \textbf{Case $\tr'(\px) ⊑ \semusg{\pe_1'}_{\tr'}$}: Then
        ...
        \begin{DispWithArrows*}
                   & \Let{\px}{\pe_1}{\Let{\py}{\pe_1'}{\pe_2'}} \Arrow{$\dead(\py,\pe_2')$, \Cref{defn:deadness3}} \\
          \equiv{} & \Let{\px}{\pe_1}{\Let{\py}{\Lam{\pz}{\pz}}{\pe_2'}} \Arrow{$\px \not∈ \fv(\Lam{\pz}{\pz})$, let floating} \\
          \equiv{} & \Let{\py}{\Lam{\pz}{\pz}}{\Let{\px}{\pe_1}{\pe_2'}} \Arrow{$\dead(\px,\pe_2')$, \Cref{defn:deadness3}} \\
          \equiv{} & \Let{\py}{\Lam{\pz}{\pz}}{\Let{\px}{\pe_2}{\pe_2'}} \Arrow{$\px \not∈ \fv(\Lam{\pz}{\pz})$, let floating} \\
          \equiv{} & \Let{\px}{\pe_2}{\Let{\py}{\Lam{\pz}{\pz}}{\pe_2'}} \Arrow{$\dead(\py,\pe_2')$, \Cref{defn:deadness3}} \\
