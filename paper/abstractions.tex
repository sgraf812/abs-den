...
\begin{definition}[Denotational deadness, lazily]
  ...
  An address $\pa$ is \emph{dead} in a denotation $\semevt{\pe}_ρ$ (written $\dead(\pa,\semevt{\pe}_ρ)$)
  if and only if, for all $d_1,d_2$, we have
  \[
    (ρ_\cong=ρ) \wedge \later(\dead(\pa,μ_\cong)) ⊦ \semevt{\pe}_{ρ_\cong}(μ_\cong[\pa↦d_1]) \cong \semevt{\pe}_{ρ_\cong}(μ_\cong[\pa↦d_2])
  \]
...
\begin{proof}
  ...
  Since $\pa$ cannot occur in $μ_\cong$, we have $\dead(\pa,μ_\cong)$.
  Since $\px$ is assumed dead in $\pe$, $\pa$ is dead in
  $\semevt{\pe}_{ρ_\cong[\px↦\pa]}$ and show the goal.
\end{proof}
...
\begin{lemmarep}[Dead let]
  \label{thm:dead-let}
  If $\px$ is dead in $\pe_r$ and $\pe$ and $\px \not= \py$, then $\px$ is also
  dead in $\Let{\py}{\pe_r}{\pe}$.
\end{lemmarep}
\begin{proof}
%  It suffices to show that the following equivalence holds for all $\pe_1$:
%  \[
%    \Let{\px}{\pe_1}{\Let{\py}{\pe_r}{\pe}} \equiv \Let{\py}{\Let{\px'}{\pe_1[\px:=\px']}{\pe_r[\px:=\px']}}{\Let{\px}{\pe_1}{\pe}}
%  \]
%  Where $\px'$ is a fresh program variable and $\pe[\py := \pz]$ denotes
%  (capture-free) substitution of $\pz$ for $\py$ inside $\pe$.
%  Clearly, for all $\pe$, $\Let{\px}{\pe_1}{\pe} \equiv \Let{\px'}{\pe_1[\px:=\px']}{\pe}$ by alpha equivalence.

We fix some $\pe_1,\pe_2$,
abbreviate $S_i \triangleq \semevt{\Let{\px}{\pe_1}{\Let{\py}{\pe_r}{\pe}}}$
and show $S_1 \cong S_2$.

We reduce the $S_i$ as follows,
abbreviating $d_i \triangleq \memo(\pa_1,\semevt{\pe_1}_{ρ[\px↦\pa_1]})$ \\
and $d_r \triangleq \memo(\pa_2, \semevt{\pe_r}_{ρ[\px↦\pa_1,\py↦\pa_2]})$:
\[\begin{array}{rcl}
  S_i(ρ)(μ) & = & \wild \cons \wild \cons \semevt{\pe}_{ρ[\px↦\pa_1,\py↦\pa_2]}(μ[\pa_1↦d_i,\pa_2↦d_r]) \\
            & = & \wild \cons \wild \cons \semevt{\pe}_{ρ[\py↦\pa_2,\px↦\pa_1]}(μ[\pa_2↦d_r,\pa_1↦d_i]) \\
            & = & \semevt{\pe}_{ρ[\py↦\pa_2,\px↦\pa_1]}(μ[\pa_2↦d_r,\pa_1↦d_i]) \\
\end{array}\]
We omit the two $\BindE$ events because they are not important.

This is equivalent to
%\[
%  \len(\semevt{\pE}{\semevt{{\Let{\py}{\pe_r}{\pe}}]}_{[]}([])) = ω
%  \Longrightarrow
%  \len(\semevt{\pE[\Let{\px}{\pe_2}{\Let{\py}{\pe_r}{\pe}}]}_{[]}([])) = ω
%\]
By induction on $\pe$.
\begin{itemize}
  \item \textbf{Case $\pe = \pz$}:

    If the $\pe_1$ term diverges, then so does the $\pe_2$ term.
\end{itemize}
\end{proof}
...
\begin{proof}
  ...
  $\tr(\px) \not⊑ \semusg{\pe}_{\tr}$''.
  %Additionally, we maintain the invariant that $\tr(\px) \not⊆ \tr(\py)$ implies that $\py$

  The goal is to show that $\px$ is dead in $\pe$.
  So we fix some $\pe_1,\pe_2$ and show that
  \[
    \Let{\px}{\pe_1}{\pe} \equiv \Let{\px}{\pe_2}{\pe}
  \]
%  So we fix some $\pe_1,\pe_2$ and $\pE$ to show
%  \[
%    \len(\semevt{\pE[\Let{\px}{\pe_1}{\pe}]}_{[]}([]))
%    =
%    \len(\semevt{\pE[\Let{\px}{\pe_2}{\pe}]}_{[]}([]))
%  \]
%  Which implies the termination property.
%  Note that
%  \[
%    \len(\semevt{\pE[\Let{\px}{\pe_i}{\pe}]}_{[]}([]))
%    =
%    \len(\semevt{\pE}(\semevt{\Let{\px}{\pe_i}{\hole}}(\semevt{\pe_i}))_{[]}([]))
%  \]

  We proceed by induction on $\pe$.

  %and consider the $ρ$ and $μ$ that
  %$\semevt{\Let{\px}{\pe_1}{\pe}}$ is called with in
  %\[
  %  \semevt{\pE[\Let{\px}{\pe_1}{\pe}]}_{[]}([]) = \semevt{\pE}(\semevt{[\Let{\px}{\pe_1}{\pe}]})_{[]}([]).
  %\]
  \begin{itemize}
    \item \textbf{Case $\pe = \Let{\py}{\pe_1'}{\pe_2'}$}:
      ...
      float out the binding for $\py$ because it might not be dead in $\px$.
      ...
      \begin{itemize}
        \item \textbf{Case $\tr'(\px) ⊑ \semusg{\pe_1'}_{\tr'}$}: Then
          ...
          \[\begin{array}{ll}
                   & \Let{\px}{\pe_1}{\Let{\py}{\pe_1'}{\pe_2'}} \\
            \equiv & \Let{\px}{\pe_1}{\Let{\py}{\Lam{\pz}{\pz}}{\pe_2'}} \\
            \equiv & \Let{\py}{\Lam{\pz}{\pz}}{\Let{\px}{\pe_1}{\pe_2'}} \\
            \equiv & \Let{\py}{\Lam{\pz}{\pz}}{\Let{\px}{\pe_2}{\pe_2'}} \\
            \equiv & \Let{\px}{\pe_2}{\Let{\py}{\Lam{\pz}{\pz}}{\pe_2'}} \\
  ...
  We proceed by induction on $\pE$, considering the $\hole$ case later.
  \begin{itemize}
    \item \textbf{Case $\pE = \pE'~\py$}:
      The stuck case can't happen because of divergence.
      So there exists $\pa' \triangleq ρ(\py)$.
      Since $\tr(\px) \not⊑ \tr(\py)$, we have $\px \not= \py ∈ \rng(ρ)$ and
      hence $\pa' \not= \pa$ (otherwise $\pa ∈ \rng(ρ)$).
