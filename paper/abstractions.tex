...
%  (Is there a weakening lemma??)
%
%  \sg{TODO: I think we need a transitive notion of deadness.
%  Perhaps it will work out with the broader definition including $L$, TODO check.}
%  Note that if $\pa$ is dead in $d$ then $d$ is $\pa$-once:
%  By \Cref{heap-partitioning}, we can partition any $μ$
%  for which $\rng(μ)$ is $\pa$-once into $μ_1 \oplus μ_2$
%  such that $\pa$ is dead in $μ_1$.
%  Then we can construct $d''(μ) \triangleq d(μ \oplus μ_2)$
%  for which
%  \[
%    \dom(μ_1) ⊦ d''(μ_1[\pa↦ d'\tick]) \lesssim\!\gtrsim d''(μ_1[\pa↦\memo(\pa,d')])
%  \]
%  URGH not that simple
