...
We can use $\noalias$ to simplify the statement ``$\px$ is dead in $\pe$'' to
showing, for some $\pa$,$d$,
\[
  \noalias(\px,\pa,ρ_\cong) \wedge \dead(\pa,μ_\cong) ⊦ \semevt{\pe}_{ρ_\cong}(μ_\cong) \cong \semevt{\pe}_{ρ_\cong}(μ_\cong[\pa↦d]),
\]
rather than to introduce a $ρ$ such that $\pa \not∈ \rng(ρ)$ just for this purpose.
\end{toappendix}
...
\begin{lemmarep}[Predicate preserving contexts are dead]
  \label{thm:ctx-preserve-dead}
  Let $\pE$ be an evaluation context and $\px$ a variable.

  If for all $ρ,μ,\pa$ such that $\noalias(\px,\pa,ρ)$ and $\dead(\pa,μ)$ it
  follows that in $\semevt{\pE}(S)_ρ(μ)$ all calls to an arbitrary $S$ are with
  a $ρ',μ'$ such that $\noalias(\px,\pa,ρ')$ and $\dead(\pa,μ')$, then $\px$
  is dead in $\pE$.
\end{lemmarep}
\begin{proof}
We abbreviate $P(ρ_\cong,μ_\cong) \triangleq \noalias(\px,\pa,ρ_\cong) \wedge \dead(\pa,μ_\cong)$
and $\diverges(τ) \triangleq \len(τ) = ω$.
We assume that $\px$ dead in some expression $\pe$ and show
\begin{DispWithArrows*}[format=Rl,fleqn,mathindent=1em]
                      & P(ρ_\cong,μ_\cong) ⊦ \semevt{\pe}_{ρ_\cong}(μ_\cong) \cong \semevt{\pe}_{ρ_\cong}(μ_\cong[\pa↦d]) \Arrow{Unfold $(\cong)$} \\
  \Longleftrightarrow & \forall \pE'. \left(\begin{array}{ll}
                                                & \diverges(\semevt{\pE'}(φ_P(\fn{ρ\,μ}{\semevt{\pe}_ρ(μ)}))_{[]}([])) \\
                            \Longleftrightarrow & \diverges(\semevt{\pE'}(φ_P(\fn{ρ\,μ}{\semevt{\pe}_ρ(μ[\pa↦d])}))_{[]}([]))
                          \end{array}\right)
                          \Arrow{$\pE' \triangleq \pE''[\pE]$} \\
  \Longrightarrow     & \forall \pE'. \left(\begin{array}{ll}
                                                & \diverges(\semevt{\pE'}(\semevt{\pE}(φ_P(\fn{ρ\,μ}{\semevt{\pe}_ρ(μ)})))_{[]}([])) \\
                            \Longleftrightarrow & \diverges(\semevt{\pE'}(\semevt{\pE}(φ_P(\fn{ρ\,μ}{\semevt{\pe}_ρ(μ[\pa↦d])})))_{[]}([]))
                          \end{array}\right)
                          \Arrow{$\pE$ preserves $P$} \\
  \Longleftrightarrow & \forall \pE'. \left(\begin{array}{ll}
                                                & \diverges(\semevt{\pE'}(φ_P(\fn{ρ\,μ}{\semevt{\pE}(\semevt{\pe})_ρ(μ)}))_{[]}([])) \\
                            \Longleftrightarrow & \diverges(\semevt{\pE'}(φ_P(\fn{ρ\,μ}{\semevt{\pE}(\semevt{\pe})_ρ(μ[\pa↦d])}))_{[]}([]))
                          \end{array}\right)
                          \Arrow{Fold $(\cong)$} \\
                      & P(ρ_\cong,μ_\cong) ⊦ \semevt{\pE}(\semevt{\pe})_{ρ_\cong}(μ_\cong) \cong \semevt{\pE}(\semevt{\pe})_{ρ_\cong}(μ_\cong[\pa↦d]) \Arrow{\Cref{thm:semevt-comp}} \\
                      & P(ρ_\cong,μ_\cong) ⊦ \semevt{\pE[\pe]}_{ρ_\cong}(μ_\cong) \cong \semevt{\pE[\pe]}_{ρ_\cong}(μ_\cong[\pa↦d])
\end{DispWithArrows*}
So $\px$ is dead in $\pE[\pe]$, too, hence $\px$ is dead in $\pE$.
\end{proof}

\begin{lemmarep}
  A variable $\px$ is dead in an expression $\pe$ if $\px$ is dead in $\pE[\pe]$
  for any dead context $\pE$.
\end{lemmarep}
\begin{proof}
We fix some $\pa,d$
and abbreviate $P(ρ_\cong,μ_\cong) \triangleq \noalias(\px,\pa,ρ_\cong) \wedge \dead(\pa,μ_\cong)$.
Note that the ``semantic context'' $E(S) \triangleq \fn{ρ\,μ}{\semevt{\pE'}(φ_P(S))_{ρ}(μ)}$
derived from an arbitrary context $\pE'$
preserves $P$, in that $P(ρ,μ)$ implies that all calls to $S$ are with $ρ',μ'$
such that $P(ρ',μ')$. (We don't even need the assumption to see that.)
Arguably, that makes $E$ a dead context since it is possible to embed a variant
of $E$ into a syntactic context $\pE$.

Equipped with this realisation, we show that $\px$ is dead in $\pe$ as follows:
\begin{DispWithArrows*}[format=Rl,fleqn,mathindent=1em]
                      & P(ρ_\cong,μ_\cong) ⊦ \semevt{\pe}_{ρ_\cong}(μ_\cong) \cong \semevt{\pe}_{ρ_\cong}(μ_\cong[\pa↦d]) \Arrow{Unfold $(\cong)$} \\
  \Longleftrightarrow & \forall \pE'. \left(\begin{array}{ll}
                                                & \diverges(\semevt{\pE'}(φ_P(\fn{ρ\,μ}{\semevt{\pe}_ρ(μ)}))_{[]}([])) \\
                            \Longleftrightarrow & \diverges(\semevt{\pE'}(φ_P(\fn{ρ\,μ}{\semevt{\pe}_ρ(μ[\pa↦d])}))_{[]}([]))
                          \end{array}\right)
                          \Arrow{$E(S) \triangleq \fn{ρ\,μ}{\semevt{\pE'}(φ_P(S))_{ρ}(μ)}$} \\
  \Longleftrightarrow & \forall \pE'. \left(\begin{array}{ll}
                                                & \diverges(E(\fn{ρ\,μ}{\semevt{\pe}_ρ(μ)})_{[]}([])) \\
                            \Longleftrightarrow & \diverges(E(\fn{ρ\,μ}{\semevt{\pe}_ρ(μ[\pa↦d])})_{[]}([]))
                          \end{array}\right)
                          \Arrow{$E$ is dead} \\
  \Longrightarrow     & \forall \pE'. \left(\begin{array}{ll}
                                                & \diverges(\semevt{\pE'}(\semevt{\pE}(φ_P(\fn{ρ\,μ}{\semevt{\pe}_ρ(μ)})))_{[]}([])) \\
...
\Cref{thm:ctx-preserve-dead} to show the goal.

So fix $ρ,μ,\pa$ such that $\noalias(\px,\pa,ρ)$ and $\dead(\pa,μ)$.
We need to show that all calls to $S$ in $\semevt{\Let{\py}{\pe_1}{\hole}}(S)_ρ(μ)$
are with some $ρ',μ'$ such that $\noalias(\px,\pa,ρ')$ and $\dead(\pa,μ')$.
...
$\dead(\pa,μ[\pa'↦\memo(\pa',\semevt{\pe_1}_{ρ[\py↦\pa']}])$, thus showing the
goal.
\end{proof}
...
\[
  \noalias(\px,\pa,ρ_\cong) \wedge \dead(\pa,μ_\cong) ⊦ \semevt{\py}_{ρ_\cong[\px↦\pa]}(μ_\cong) \cong \semevt{\py}_{ρ_\cong[\px↦\pa]}(μ_\cong[\pa↦d])
\]
...
\[
  \noalias(\px,\pa,ρ_\cong) \wedge \dead(\pa,μ_\cong) ⊦ \later(\memo(\pa',\semevt{\pe'}_{ρ'})(μ_\cong) \cong \memo(\pa',\semevt{\pe'}_{ρ'})(μ_\cong[\pa↦d]))
\]
...
occurrences of $\pe'$.
%XXX: This argument doesn't hold up for improvement, I think

For a value $\pv$, $\memo$ has no observable effect and we can simply discard
the extra transition.
The goal becomes
\[
  \noalias(\px,\pa,ρ_\cong) \wedge \dead(\pa,μ_\cong) ⊦ \later(\semevt{\pv}_{ρ'})(μ_\cong) \cong \semevt{\pv}_{ρ'})(μ_\cong[\pa↦d]))
\]
...
\begin{figure}
\[\begin{array}{c}
 \ruleform{ \dead(V,\pe) \qquad \dead(V,\pE) }
 \\
 \\[-0.5em]
 \inferrule*[right=]
    {\many{\dead(\px,\pe)}}
 ...
 \inferrule*[right=]
    {\dead(V,\pE_1) \quad x \not∈ V}
    {\dead(V,\Let{\px}{\pE_1}{\pE_2[\px]})}
 \\
 \\[-0.5em]
\end{array}\]
\label{fig:correct-deadness}
\caption{Correctness relation for \Cref{thm:semusg-correct-live-3}}
\end{figure}

\begin{lemmarep}
  If $\dead(V, \pE)$, then for all $\pe$ such that $\dead(V,\pe)$, $\dead(V,\pE[\pe])$.
\end{lemmarep}
\begin{proof}
By induction on $\pE$.
\begin{itemize}
  \item \textbf{Case $\pE = \hole$}: By assumption.
  \item \textbf{Case $\pE = \pE'~\py$}:
    By $\dead(V,\pE'~\py)$, we know that $\py ∈ V$.
    By the induction hypothesis, $\dead(V,\pE'[\pe])$.
...
\begin{itemize}
    ...
    \begin{itemize}
      \item \textbf{Case $\tr'(\px) ⊑ \semusg{\pe_1'}_{\tr'}$}: Then
        ...
        \begin{DispWithArrows*}
                   & \Let{\px}{\pe_1}{\Let{\py}{\pe_1'}{\pe_2'}} \Arrow{$\dead(\py,\pe_2')$, \Cref{thm:dead-irrelevant}} \\
          \equiv{} & \Let{\px}{\pe_1}{\Let{\py}{\Lam{\pz}{\pz}}{\pe_2'}} \Arrow{$\px \not∈ \fv(\Lam{\pz}{\pz})$, let floating} \\
          \equiv{} & \Let{\py}{\Lam{\pz}{\pz}}{\Let{\px}{\pe_1}{\pe_2'}} \Arrow{$\dead(\px,\pe_2')$, \Cref{thm:dead-irrelevant}} \\
          \equiv{} & \Let{\py}{\Lam{\pz}{\pz}}{\Let{\px}{\pe_2}{\pe_2'}} \Arrow{$\px \not∈ \fv(\Lam{\pz}{\pz})$, let floating} \\
          \equiv{} & \Let{\px}{\pe_2}{\Let{\py}{\Lam{\pz}{\pz}}{\pe_2'}} \Arrow{$\dead(\py,\pe_2')$, \Cref{thm:dead-irrelevant}} \\
          \equiv{} & \Let{\px}{\pe_2}{\Let{\py}{\pe_1'}{\pe_2'}}
        \end{DispWithArrows*}
        as requested.
      \item \textbf{Case $\tr'(\px) \not⊑ \semusg{\pe_1}_{\tr'}$}:
        Then $\px$ is dead both in $\pe_1'$ and $\pe_2'$ by the induction
        hypothesis and the result follows by \Cref{thm:dead-let}.
    \end{itemize}

  \item \textbf{Case $\pe = \pe'~\py$}:
    Since $\tr(\px) \not⊑ \tr(\py)$, we have $\px \not= \py$ and
    by \Cref{thm:var-dead}, $\px$ is dead in $\py$.
    It is also $\tr(\px) \not⊑ \semusg{\pe'}_\tr$, so $\px$ is dead in $\pe'$.

    To show that $\px$ is dead in $\pe'~\py$ it suffices to show that $\px$
    is dead in the context $\hole~\py$, assuming that $\px \not= \py$, by
    \Cref{thm:ctx-preserve-dead}.

    Hence we assume for some $ρ,μ,\pa$ that $\noalias(\px,\pa,ρ)$
    and $\dead(\pa,μ)$ and have to show that the calls to $S$ in
    $\semevt{\hole~\py}(S)_ρ(μ)$ are such that $\noalias$ and $\dead$ follows.
    Since the only call to $S$ is with an unchanged $ρ$ and $μ$, the goal
    follows immediately.
  ...
  \item \textbf{Case $\pe = \Lam{\py}{\pe'}$}:
    ...
    to show
    \[\begin{array}{ll}
                            & P(ρ_\cong,μ_\cong) ⊦ \semevt{\Lam{\py}{\pe'}}_{ρ_\cong}(μ_\cong) \cong \semevt{\Lam{\py}{\pe'}}_{ρ_\cong}(μ_\cong[\pa↦d]) \\
      \Longleftrightarrow{} & \forall \pE. \left(\begin{array}{ll}
