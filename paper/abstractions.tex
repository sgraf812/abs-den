...
%  \]
  ...
  \[
    \len(\semevt{\pE[\Let{\px}{\pe_1}{\pe}]}_{[]}([])) = ω
    \Longleftrightarrow
    \len(\semevt{\pE[\Let{\px}{\pe_2}{\pe}]}_{[]}([])) = ω
  \]
  Since $\pe_i$ were chosen arbitrary and the predicate is symmetric, it
  suffices to show one direction.
  % Furthermore, we generalise the empty environment and heap to arbitrary
  % $ρ$ and $μ$.
  So let us assume that $\semevt{\pE[\Let{\px}{\pe_1}{\pe}]}_{[]}([])$
  diverges and show that $\semevt{\pE[\Let{\px}{\pe_2}{\pe}]}_{[]}([])$ does, too.
