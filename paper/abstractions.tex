...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item \textbf{Case $\pe = \pe'~\py$}:
      ...
      Otherwise, $\tr(\py)(\px) = 0$ and hence
      $\ctx_D(\tm(ρ_1(\py))(\tm))(\pa) = 0$ by \Cref{eqn:semusg-rho}.
      We apply the induction hypothesis to $D ∪ \{ ρ_1(\py) \}$ to see that
      \[
        \ctx_{D ∪ \{ ρ_1(\py) \}}(\usg_\EventD(\semevt{\pe'}_{ρ_1})(\tm))(\pa) ⊑ \semusg{\pe'}_{\tr}(\px).
      \]
      Then we have $\usg_\EventD(\semevt{\pe'}_{ρ_1})(\tm) = l \lcons \someend{\FunV(\tilde{f}),\tm'}$ and
      $ρ(\py) ∈ D ∪ \{ ρ_1(\py) \}$, so
      \[
        l + ω*\tilde(f)(ρ(\py)) ⊑ \ctx_{D ∪ \{ ρ_1(\py) \}}(\usg_\EventD(\semevt{\pe'}_{ρ_1})(\tm))(\pa) ⊏ \semusg{\pe'}_{\tr}(\px).
      \]
      But then also
      \[
        u_l = l + \tilde(f)(ρ(\py)) ⊑ \semusg{\pe'}_{\tr}(\px) + \tr(\py)(\px) = u_r
      \]
